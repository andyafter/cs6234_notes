...
\begin{frame}
  ...
  \only<2>{
    ...
    \item Armijo Condition:
      $$f(\xk{k} + \alpha {\bf p}^{(k)}) \le f(\xk{k}) + c_1\alpha\nabla
      f(\xk{k})^T {\bf p}^{(k)}, c_1 > 0$$
    \item Wolfe Conditions (Including Armijo Condition):
      $$\nabla f(\xk{k} + \alpha {\bf p}^{(k)})^T{\bf p}^{(k)} \ge
      c_2\nabla f(\xk{k})^T{\bf p}^{(k)}, 0< c_1 < c_2 < 1$$
    \end{itemize}

Where ${\bf p}$ is the step direction.
  ...
  \only<3>{
    ...
    \begin{greenblock}{Algorithm:}
      \begin{algorithm}[H]
        ...
        \While{$f(\xk{k} + t\Delta \xk{k}) \le f(\xk{k}) + \alpha t\nabla
          f(\xk{k})^T\Delta \xk{k}$}{
          $t =\beta t$\;
        }
        % \caption{Descent Method Algorithm}
